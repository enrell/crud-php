...
class PatientRepository {
    ...
    public function create(string $name, string $birthdate, string $bloodType): bool {
        try {
            requireAuth();
            
            $patient = new Patient($name, $birthdate, $bloodType);
            
            $stmt = $this->db->prepare("INSERT INTO patient (name, birthdate, blood_type) VALUES (?, ?, ?)");
            $result = $stmt->execute([
                $patient->getName(), 
            ...
            error_log("PatientRepository Error: " . $e->getMessage(), 3, __DIR__ . '/../../debug.log');
            return false;
        }
    }
    ...
    public function update(int $id, string $name, string $birthdate, string $bloodType): bool {
        try {
            requireAuth();
            ...
            $stmt = $this->db->prepare("UPDATE patient SET name = ?, birthdate = ?, blood_type = ? WHERE id = ?");
            $result = $stmt->execute([
                $patient->getName(), 
                $patient->getBirthdate(), 
                $patient->getBloodType(), 
