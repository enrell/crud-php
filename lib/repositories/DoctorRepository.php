...
class DoctorRepository {
    ...
    public function create(string $name, string $expertise): bool {
        try {
            ...
            $stmt = $this->db->prepare("INSERT INTO doctor (name, expertise) VALUES (?, ?)");
            $result = $stmt->execute([
                $nameObj->getValue(), 
                $expertise
            ]);
            ...
            error_log("DoctorRepository Error: " . $e->getMessage(), 3, __DIR__ . '/../../debug.log');
            return false;
        }
    }
    ...
    public function update(int $id, string $name, string $expertise): bool {
        try {
            ...
            $entityId = new EntityId($id);
            $nameObj = new Name($name);
            $stmt = $this->db->prepare("UPDATE doctor SET name = ?, expertise = ? WHERE id = ?");
            $result = $stmt->execute([
                $nameObj->getValue(), 
                $expertise, 
                $entityId->getValue()
